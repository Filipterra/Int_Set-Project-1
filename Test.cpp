 ...
 *
 *  Description : Simple tests of Class_Set functionality
 */
...
//Randomized assignment of n integers to the given table
...
bool Test5()
{
    ...
    for (unsigned int i=0; i<TABLE_SIZE; i++) A=A-T[i];;
    for (unsigned int i=0; i<TABLE_SIZE; i++) if (*(A.Elements+T[i]+MAX_INT_VAL)>0)
        {
            h=0;
            break;
        }
...
bool Test8()
{
    ...
    for (unsigned int i=0; i<TABLE_SIZE; i++) if (*(A.Elements+T[i]+MAX_INT_VAL)>0)
        {
            /*cout<<B<<endl<<T[i]-MAX_INT_VAL<<"="<<*(A.Elements+T[i]-MAX_INT_VAL)<<endl;*/ h=0;
            break;
        };
