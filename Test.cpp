...
Set& A=*(new Set(0));
Set& B=*(new Set(0));
for (int i=0; i<TABLE_SIZE; i++) A=A+T[i];
for (int i=0; i<TABLE_SIZE; i++) B+=T[i];
if (A==B) h=1;
...
Set& A=*(new Set(0));
Set& B=*(new Set(0));
for (int i=0; i<TABLE_SIZE; i++) A+=T[i];
B=B+A;
if (A==B) h=1;
...
Fill_rand(T,TABLE_SIZE);
Set& A=*(new Set(0));
for (int i=1; i<MAX_INT_VAL*2; i++) *(A.Elements+i)=1;
for (int i=0; i<TABLE_SIZE; i++) A=A-T[i];;
for (int i=0; i<TABLE_SIZE; i++) if (*(A.Elements+T[i]+MAX_INT_VAL)>0) {h=0; break;}
...
for (int i=0; i<TABLE_SIZE; i++) B+=T[i];
A-=B;
for (int i=0; i<TABLE_SIZE; i++) if (*(A.Elements+T[i]+MAX_INT_VAL)>0) {/*cout<<B<<endl<<T[i]-MAX_INT_VAL<<"="<<*(A.Elements+T[i]-MAX_INT_VAL)<<endl;*/ h=0; break;};
...
Set& A=*(new Set(0));
Set& B=*(new Set(0));
for (int i=1; i<MAX_INT_VAL*2; i++) *(A.Elements+i)=TABLE_SIZE+1;
for (int i=0; i<TABLE_SIZE; i++) B+=T[i];
A=A*B;
if (A==B) h=1;
